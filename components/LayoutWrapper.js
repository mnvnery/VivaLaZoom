...
return (
    ...
    <div className="flex h-screen flex-col justify-between">
        ...
        <div className="flex justify-center min-h-[11vh]">
            <div className={`${isOpen ? 'fixed top-0 left-0 flex justify-center items-center h-screen w-screen z-20' : 'z-[1] mb-[-1em]'}`}>
                <motion.div layout onClick={() => setIsOpen(!isOpen)} className={`bg-pink ${isOpen ? 'min-h-[20px] min-w-[10em] rounded-xl mx-10 md:mx-0' : 'rounded-t-xl' }`}>
                    {!isOpen &&
        ...
        <Footer />
        <div className={`h-screen w-screen fixed top-0 left-0 z-10 pointer-events-none bg-white ${isOpen ? 'opacity-75 duration-300' : 'opacity-0 duration-75'}`}></div>
    </div>
    </>
)
}